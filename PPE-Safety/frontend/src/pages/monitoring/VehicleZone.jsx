 ...
 *
 * The Restricted Zone page's sibling, and deliberately the same page: freeze a
 * frame, mark the floor, watch it. The two areas are kept apart in the backend
 * because a forklift aisle and a people-exclusion strip are frequently the
 * inverse of each other, and they are kept apart here for the same reason.
 *
 * What this page carries that its sibling does not is a statement of what the
 * weights can actually report, because on this module that is not a footnote.
 * The supplied model has one class and it is `forklift` — not "vehicle" — and
 * it over-triggers: measured across four clips containing no forklift at all,
 * at least one detection came back on 19–85% of frames at 0.25 confidence, and
 * the six most confident detections anywhere (0.816–0.844) are five views of a
 * worker's forearm and one of a person sitting at a desk.
 *
 * The backend answers that with a confidence floor set above every false
 * positive that footage produced, and publishes both the number and how it was
 * arrived at as `confidence_note`. This page's job is to put that sentence
 * where the person changing the number will read it, rather than leaving it in
 * a docstring nobody on shift opens.
 */

const api = createModuleApi("vehicle-zone");
const POLL_MS = 3000;

/**
 * Confidence floors offered on screen, as fractions.
 *
 * Coarse on purpose. The useful range is narrow and measured: at 0.90 not one
 * frame of the four forklift-free clips produced a detection, and at 0.25 most
 * of their frames did. Offering 0.01 steps would suggest a precision the
 * measurement cannot support, so these are landmarks either side of the floor
 * the backend chose.
 */
const CONFIDENCE_CHOICES = [0.5, 0.7, 0.85, 0.9, 0.95];
...
export default function VehicleZone() {
  ...
  const webcam = useWebcamAnalysis("vehicle-zone", { file: videoUrl });
  ...
  // the ability to see a vehicle standing in it, and "Area clear" over a
  ...
  const sound = useAlertSound(alert, results?.summary, {
    ...
    unverified: {
      ...
      spoken: unverifiedSpeech("the vehicle restricted zone", reason),
      resumed:
        watching && !unreadable
          ? resumedSpeech("The vehicle restricted zone")
          : null,
    },
  });
  ...
  const classes = config?.classes?.length ? config.classes : ["forklift"];
  ...
  return (
    <ModuleLayout
      ...
      description="The AI watches the marked area and alerts while a forklift is standing in it. Forklifts only — no other vehicle is recognised."
      icon={Forklift}
      watching={watching}
      alert={alert}
      ...
      side={
        <>
          ...
          <Panel title="Marked area" icon={Pentagon}>
            {hasZone && unreadable ? (
              /* The area is still marked, but "a forklift standing inside it
              ...
              <UnverifiedNotice
                reason={reason}
                description="The area is still marked, but a forklift standing in it would not be spotted."
              />
            ) : hasZone ? (
              ...
              <div className="space-y-2 text-sm">
                ...
                <p className="text-xs text-text-secondary">
                  {watching
                    ? "A forklift standing inside it raises an alert."
              ...
              <EmptyState
                ...
                description={
                  watching
                    ...
                    : "Nothing is being watched. Start watching a camera first, then mark the floor a forklift must stay out of."
                }
              />
            )}
          </Panel>
            ...
            <div className="space-y-3">
              ...
              {belowMeasuredFloor && (
                <p className="text-xs text-warning font-medium">
                  ...
                  measured on forklift-free footage start coming back.
                </p>
              )}
          ...
          <Panel title="What this AI can see" icon={Info}>
            <div className="space-y-3 text-xs text-text-secondary leading-relaxed">
              ...
              <p>
                A pallet truck, a tug, a crane or a van is not seen at all, so
                this page never claims to watch vehicles in general.
              </p>
              ...
              <p>
                It also over-triggers. On four clips containing no forklift, a
                sighting came back on{" "}
                <span className="font-medium text-text">19–85% of frames</span>{" "}
                at 25% confidence, and the six most confident detections
                anywhere — 82% to 84% — were five views of a worker&apos;s
                forearm and one of a person sitting at a desk.
              </p>

              <p>
                The {Number.isFinite(confidenceDefault) ? percent(confidenceDefault) : "default"}{" "}
                floor sits above every one of those. What it costs in real
                forklifts is not known: no footage containing one exists here
                yet, so treat an alert as something to go and look at.
              </p>
            </div>
          </Panel>

          <Panel title="Past vehicle events" icon={TriangleAlert}>
            {/* Refreshed when the alert state changes, so something spotted
                while the operator is watching appears without a reload. */}
            <RecentEvents moduleId="vehicle-zone" refreshToken={alert ? 1 : 0} />
          </Panel>
        </>
      }
    >
      <StatusCard
        ...
        title={
          modelMissing
            ? "Vehicle monitoring is not available"
            : alert
              ? results.summary
              : unreadable
                ? reason
        ...
        description={
          modelMissing
            ? "The forklift AI is not installed on this system."
            : alert
              ? "A forklift is standing in the marked area. Check the live view and respond according to your site's safety procedure."
              : unreadable
                ? unverifiedDescription(
                    hasZone
                      ? "The marked area is not being watched — a forklift could be standing in it."
                      : "Nothing is being watched.",
                  )
                : !hasZone
                  ? "Nothing is watched until an area is marked. Connect a camera, then mark the floor a forklift must stay out of."
                  : !watching
                    ? "Connect a camera to begin watching the marked area."
                    : judged
                      ? "The marked area is being watched. You'll be alerted while a forklift is standing in it."
      ...
      {judged && !modelMissing && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          <StatisticsCard
            label="Forklifts in view"
            value={measuredCount(results.vehicles_total ?? 0, unreadable)}
            icon={Forklift}
            tone="neutral"
            hint={
              unreadable
          ...
          <StatisticsCard
            ...
            value={measuredCount(results.vehicles_inside ?? 0, unreadable)}
            icon={Pentagon}
            tone={
              unreadable
                ? "neutral"
                : results.vehicles_inside > 0
                  ? "danger"
