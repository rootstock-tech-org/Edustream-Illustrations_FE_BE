...
export function SaveButton({ item, onDark = false }: { item: SaveItem; onDark?: boolean }) {
  ...
  const base = onDark
    ...
    : "bg-[var(--panel)]/80 text-[var(--text-secondary)] hover:bg-[var(--panel)] hover:text-[var(--text)] ring-1 ring-[var(--border-strong)]";
  ...
  return (
    <button
      ...
      className={`grid h-8 w-8 place-items-center rounded-full backdrop-blur transition ${base} ${saved ? "!text-[var(--signal)]" : ""}`}
    >
